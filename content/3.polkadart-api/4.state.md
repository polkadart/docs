...
#### getTypedMetadata
```dart
Future<TypedMetadata> getTypedMetadata({BlockHash? at})
```
Fetch typed metadata with full type information for code generation and advanced queries.

**Use case:** Required for generating metadata hashes (RFC-78), type-safe storage decoding, and code generation tools.

**Example:**
```dart
final stateApi = StateApi(provider);
final typedMetadata = await stateApi.getTypedMetadata();

// Access type registry
print('Types count: ${typedMetadata.types.length}');
```
